...
本模型构建了一个基于博弈论的网约车司机动态定价策略模型，通过模拟多轮博弈过程，研究司机间的策略互动、学习适应和均衡演化过程。模型融合了深度强化学习(DQN)和长短期记忆网络(LSTM)技术，使智能体能够在复杂的市场环境中学习最优定价策略。
...
1. **市场状态生成**
   - 根据当前时间、地点、天气等因素生成市场状态
   - 计算当前市场需求和供给水平
   - 生成订单池，包含订单距离、价格敏感度等特征

2. **智能体决策**
   - 每个智能体观察当前市场状态和对手历史策略
   - 将观察转化为状态向量(包含15-20个特征)
   - 通过DQN网络选择动作(价格策略，范围10-50元)
   - 应用探索-利用策略(ε-greedy)平衡探索与利用

3. **订单分配与执行**
   - 根据司机定价策略和订单特征进行匹配
   - 使用概率模型模拟用户选择行为(价格敏感度、距离偏好等)
   - 计算每个司机获得的订单数量和特征
...
4. **收益计算**
   ...
   - 其中成本包括：
     * 基础成本 = 固定成本 + 距离成本 × 总距离
     * 机会成本 = 未接单时间 × 时间单位成本
     * 竞争成本 = f(自身价格, 对手价格) × 市场竞争系数

5. **奖励计算**
   - 奖励函数设计如下：
   ```
   奖励 = α × 绝对收益 + β × 相对收益 + γ × 策略稳定性奖励 - δ × 极端策略惩罚
   ```
   - 参数设置：α=0.7, β=0.2, γ=0.05, δ=0.1
   - 相对收益 = (自身收益 - 对手收益) / 100
   - 策略稳定性奖励 = 如果策略变化小于阈值，给予小额正奖励
   - 极端策略惩罚 = 如果策略<15或>45，给予负奖励
