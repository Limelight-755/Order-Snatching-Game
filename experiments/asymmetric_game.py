...
class AsymmetricGameExperiment:
    ...
    def _run_single_experiment(self) -> List[RoundResult]:
        ...
        for round_num in range(1, self.config.total_rounds + 1):
            ...
            # 计算奖励（不同的奖励函数）
            reward_a = self._calculate_asymmetric_reward(
                revenue_a, revenue_b, strategy_a, 'player_a'
            )
            reward_b = self._calculate_asymmetric_reward(
                revenue_b, revenue_a, strategy_b, 'player_b'
            )
            
            # 更新智能体
            next_market_state = self.market_env.get_current_state()
            next_state_vector_a = self._prepare_asymmetric_state_vector(
                next_market_state, strategy_history_b + [strategy_b], 'player_a'
            )
            next_state_vector_b = self._prepare_asymmetric_state_vector(
                next_market_state, strategy_history_a + [strategy_a], 'player_b'
            )
            
            self.player_a.store_experience(
                state_vector_a, action_a, reward_a, next_state_vector_a, False
            )
            self.player_b.store_experience(
                state_vector_b, action_b, reward_b, next_state_vector_b, False
            )
            
            # 不同的训练频率
            if round_num % 15 == 0:  # 玩家A训练频率较低
                self.player_a.train()
            if round_num % 8 == 0:   # 玩家B训练频率较高
                self.player_b.train()
    ...
    def _plot_asymmetric_strategy_evolution(self, result: ExperimentResult):
        ...
        # 策略演化
        plt.subplot(2, 1, 1)
        plt.plot(rounds, strategies_a, label='玩家A策略（弱势）', alpha=0.7, color='blue')
        plt.plot(rounds, strategies_b, label='玩家B策略（强势）', alpha=0.7, color='red')
        ...
        # 策略差异
        plt.subplot(2, 1, 2)
        strategy_diff = [b - a for a, b in zip(strategies_a, strategies_b)]
        plt.plot(rounds, strategy_diff, label='策略差异（B-A）', alpha=0.7, color='green')
    ...
    def _plot_learning_curves(self, result: ExperimentResult):
        ...
        for i in range(len(revenues_a)):
            avg_a = np.mean(revenues_a[:i+1])
            avg_b = np.mean(revenues_b[:i+1])
            cumulative_avg_a.append(avg_a)
            cumulative_avg_b.append(avg_b)
        
        plt.figure(figsize=(12, 6))
        plt.plot(rounds, cumulative_avg_a, label='玩家A学习曲线', alpha=0.8, color='blue')
        plt.plot(rounds, cumulative_avg_b, label='玩家B学习曲线', alpha=0.8, color='red')
        
        plt.xlabel('轮次')
        plt.ylabel('累积平均收益（元）')
        plt.title('非对称博弈学习曲线对比')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        plot_path = f"results/plots/asymmetric_learning_curves_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        plt.savefig(plot_path)
        plt.close()
